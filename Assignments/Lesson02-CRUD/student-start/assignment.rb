...
class Solution
  ...
  def self.collection
    return mongo_client[ENV['RACE_COLLECTION'] || RACE_COLLECTION]
  end
  ...
  def self.mongo_client
    @@db = Mongo::Client.new(ENV['MONGO_URL'] || MONGO_URL)
             .use(ENV['MONGO_DATABASE'] || MONGO_DATABASE)
  end
  ...
  def find_by_name(fname, lname)
    @coll
      .find(first_name: fname, last_name: lname)
      .projection(_id: 0, first_name: 1, last_name: 1, number: 1)
  end
  ...
  def find_by_letter(letter, offset, limit)
    @coll
      .find(last_name: {:$regex => /^#{letter.upcase}.+/})
      .sort(last_name: 1)
      .skip(offset)
      .limit(limit)
  end
